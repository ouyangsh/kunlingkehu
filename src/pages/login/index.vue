...
<template>
  <view class="fixed inset-0 flex flex-col items-center px-40rpx overflow-hidden bg-[#fffcfc]">
    <!-- Custom Header -->
    <view
      class="w-full h-88rpx flex items-center justify-center flex-shrink-0"
    ...
    <view class="w-full flex-1 flex flex-col items-center justify-center min-h-0 py-30rpx">
      <view
        class="w-400rpx h-520rpx rounded-32rpx overflow-hidden shadow-lg border-[12rpx] border-white relative flex-shrink-0"
      >
        <image
    ...
    <view class="w-full bg-white rounded-40rpx p-50rpx mb-40rpx flex-shrink-0">
      <uv-form ref="emailFormRef" :model="emailForm" :rules="emailRules" labelWidth="0">
        <uv-form-item prop="email" :border-bottom="false" :customStyle="{ paddingBottom: '10rpx' }">
          <view
            class="bg-[#fcfafa] rounded-24rpx px-30rpx py-8rpx min-h-88rpx flex items-center w-full"
          >
            <view class="flex-1">
              <uv-input
        ...
        <uv-form-item prop="code" :border-bottom="false" :customStyle="{ paddingTop: '10rpx' }">
          <view
            class="bg-[#fcfafa] rounded-24rpx px-30rpx py-8rpx flex items-center w-full min-h-88rpx"
          >
            <view class="flex-1">
              <uv-input
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (accessToken) {
      ...
      // Handle redirection with explicit flag check
      const isNewUser = !!res.data.is_new_user
      console.log('Login success, isNewUser:', isNewUser, res.data)

      setTimeout(() => {
        if (isNewUser) {
          console.log('Redirecting new user to manage page...')
          uni.reLaunch({
            url: '/pages/index/manage?openCreate=1',
            success: () => console.log('ReLaunch to manage success'),
            fail: (e) => console.error('ReLaunch to manage failed', e),
          })
        } else {
